...
public interface CashMovementRepository extends JpaRepository<CashMovement, Long> {
    ...
    @Query("SELECT COALESCE(SUM(m.amount), 0) FROM CashMovement m WHERE m.type IN :types AND m.createdAt >= :from AND m.createdAt < :to AND m.active = true")
    BigDecimal sumByTypes(@Param("types") Collection<CashMovementType> types,
                          @Param("from") LocalDateTime from,
                          @Param("to") LocalDateTime to);

    @Query("SELECT COUNT(m.id) FROM CashMovement m WHERE m.createdAt >= :from AND m.createdAt < :to AND m.active = true")
